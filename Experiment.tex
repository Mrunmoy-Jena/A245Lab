...
\end{equation}
We take initial guess parameters as: $a=1/500\ \mu\rm{W}^{-1}$ (we used this because, $x$ value of 100 gives a $y$ value of around 20; assuming quadratic dependence, this would give the factor to be multiplied with $x^{2}$ as $20/10000=1/500$), $b=0$ (linear dependence is assumed to be very small) and $c=0\ \mu\rm{W}$ (no offset can be seen from the $x$-axis). The curve fitting module yields the following optimal parameter values:
\begin{align*}
...
These two models are then made to fit onto the observed data and the two fits are compared. It can be seen clearly from the plot that the model for Type I phase matching fits the data much better than the model for Type II phase matching. Although the reduced $\chi_{red}^{2}$ are extremely high for the models ($\chi_{red}^{2}=278.54$ for Type I and $\chi_{red}^{2}=1647.05$ for Type II), this comes as no surprise as even for the $\cos^{4}\theta$ many of the data points lie just outside the best fit curve. Moreover, the factors discussed at the end of Section \ref{chisqdiscuss} also play a role in these large values of $\chi_{red}^{2}$. Despite all this, since the $\chi_{red}^{2}$ for the $\cos^{4}\theta$ model is much lower than that for the $\cos^{2}\theta\sin^{2}\theta$ model, we conclude that the data is best described by $\cos^{2}\theta$ model and therefore Type I phase matching has been attained. The initial guess parameters for the $\cos^{2}\theta$ fit were chosen as: $a=50\ \mu\rm{W}$, $b=2$, $\theta_{0}=25\degree$ and $c=0\ \mu \rm{W}$. The optimal parameters of this fit are:
\begin{align*}
a&=(40.671\pm 0.032)\ \mu\rm{W}\\
b&=1.99278\pm 0.00042\\
c&=(11.363\pm 0.017)\ \mu \rm{W}\\
\theta_{0}&=(25.269\pm 0.015)\degree
\end{align*}
Physically, the parameter $a$ represents the maximum value of power recorded, $b$ indicates the periodicity of the $\cos^{4}\theta$ variation, $c$ gives the offset reading and $\theta_{0}$ gives the phase of the modelled function. It is nice to see that the periodicity, $b$ and phase, $c$ remain nearly the same as the values obtained during calibration in Section \ref{type1ornot}. This again confirms that we had indeed achieved Type I phase matching for this experiment.
